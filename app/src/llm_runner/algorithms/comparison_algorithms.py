...
try:
    ...
    from deepeval import evaluate  # type: ignore[import-untyped]
...
def load_dataset_with_cache(
    ...
    if DATA_MANAGER_AVAILABLE:
        ...
        return load_dataset(dataset_name, **kwargs)  # type: ignore[call-overload]
    else:
...
class ComparisonAlgorithm(ABC):
    ...
    @abstractmethod
    def compare(
        self, responses: List[ModelResponse], reference_data: Optional[Any] = None
    ...
    def compare(
        ...
        try:
            ...
            for i, response in enumerate(responses):
                ...
                reference = (
                    reference_data[i] if i < len(reference_data) else reference_data[0]
                )
    ...
    def compare(
        ...
        for i, response in enumerate(responses):
            ...
            if self.scorer is None:
                raise RuntimeError("ROUGE scorer not initialized")
...
class SemanticSimilarityAlgorithm(ComparisonAlgorithm):
    ...
    def compare(
        ...
        if not self.is_available():
            raise RuntimeError(
                "Semantic similarity not available - install sentence-transformers"
            )
        ...
        try:
            ...
            for i, response in enumerate(responses):
                ...
                score = (
                    similarity + 1
                ) * 50  # cosine similarity is -1 to 1, convert to 0-100
...
class BERTScoreAlgorithm(ComparisonAlgorithm):
    ...
    def __init__(self):
        super().__init__(
            name="BERTScore", description="BERTScore for semantic similarity evaluation"
        )
    ...
    def compare(
        ...
        try:
            ...
            for i, response in enumerate(responses):
                ...
                r_val = R[i]
                f1_val = F1[i]
                # Type ignore needed for BERTScore tensor operations
                precision = p_val.item() if hasattr(p_val, "item") else float(p_val)  # type: ignore[union-attr]
                recall = r_val.item() if hasattr(r_val, "item") else float(r_val)  # type: ignore[union-attr]
                f1_score = f1_val.item() if hasattr(f1_val, "item") else float(f1_val)  # type: ignore[union-attr]
...
class PairwiseComparisonAlgorithm(ComparisonAlgorithm):
    ...
    def compare(
        ...
        if len(responses) < 2:
            logger.info(
                f"Pairwise comparison requires at least 2 responses (got {len(responses)}), returning neutral scores"
            )
            # Return neutral results for single response
            for response in responses:
                results.append(
                    ComparisonResult(
                        algorithm_name=self.name,
                        model_id=response.model_id,
                        score=50.0,  # Neutral score (50/100)
                        additional_metrics={
                            "note": "Single response - no comparison possible",
                            "comparison_type": "single_response",
                        },
        ...
        if self.reference_loader:
            try:
                ...
                if alpacaeval_data:
                    ...
                    logger.warning(
                        "AlpacaEval dataset is empty, falling back to heuristics"
                    )
            except Exception as e:
                logger.warning(
                    f"Could not load AlpacaEval data, falling back to heuristics: {e}"
                )

        # Fallback to heuristic pairwise comparison
        return self._evaluate_with_heuristics(responses)

    def _evaluate_with_alpacaeval(
        self, responses: List[ModelResponse], alpacaeval_data: List[dict]
    ) -> List[ComparisonResult]:
        ...
        for i, response_i in enumerate(responses):
            ...
            for j, response_j in enumerate(responses):
                if i != j:
                    ...
                    preference_score = self._alpacaeval_preference(
                        response_i, response_j, sample_pairs
                    )
    ...
    def _alpacaeval_preference(
        ...
        for pair in sample_pairs:
            ...
            score_a = self._alpacaeval_response_quality(
                response_a.response, instruction, reference_output
            )
            score_b = self._alpacaeval_response_quality(
                response_b.response, instruction, reference_output
            )
    ...
    def _alpacaeval_response_quality(
        self, response: str, instruction: str, reference: str
    ) -> float:
        ...
        instruction_overlap = (
            len(instruction_words & response_words) / len(instruction_words)
        ...
        if reference:
            ...
            response_words = set(response.lower().split())
            similarity = (
                len(ref_words & response_words) / len(ref_words | response_words)
                if ref_words | response_words
                else 0
            )
    ...
    def _evaluate_with_heuristics(
        ...
        win_matrix = (
            ...
            if NUMPY_AVAILABLE
            else [[0] * n_models for _ in range(n_models)]
        )
        ...
        for i, response_i in enumerate(responses):
            ...
            for j, response_j in enumerate(responses):
                if i != j:
                    ...
                    preference_score = self._calculate_preference(
                        response_i, response_j
                    )
                    ...
                    if NUMPY_AVAILABLE:
                        win_matrix[i][j] = preference_score  # type: ignore
                    else:
                        win_matrix[i][j] = preference_score  # type: ignore
    ...
    def _calculate_preference(
        self, response_a: ModelResponse, response_b: ModelResponse
    ) -> float:
        ...
        factors.append(
            len_score_a / (len_score_a + len_score_b)
            if len_score_a + len_score_b > 0
            else 0.5
        )
        ...
        factors.append(
            quality_a / (quality_a + quality_b) if quality_a + quality_b > 0 else 0.5
        )
...
Evaluation (1-10): """
    ...
    def compare(
        ...
        if self.reference_loader:
            try:
                    ...
                    results.append(
                        ComparisonResult(
                            ...
                            additional_metrics={
                                ...
                                "mtbench_samples": (
                                    len(mtbench_data) if mtbench_data else 0
                                ),
                ...
                logger.warning(
                    f"Could not load MT-Bench data, falling back to heuristics: {e}"
                )
    ...
    def _simulate_llm_judgment(self, response: ModelResponse) -> float:
        ...
        relevance_score = min(
            10.0, len(text) / 20
        )  # Longer responses assumed more relevant

        # Clarity: penalize very short or very long responses
        length = len(text.split())
        clarity_score = (
            10.0 if 10 <= length <= 200 else max(1.0, 10.0 - abs(length - 100) / 20)
        )
        ...
        if any(
            word in text.lower()
            for word in ["example", "because", "how", "why", "what"]
        ):
            helpfulness_score += 2.0

        # Average the scores
        overall_score = (
            relevance_score + clarity_score + accuracy_score + helpfulness_score
        ) / 4
        return min(10.0, max(1.0, overall_score))

    def _evaluate_with_mtbench(
        self, response: ModelResponse, mtbench_data: List[dict]
    ) -> float:
        ...
        for conversation in sample_conversations:
            ...
            turn_score = self._mtbench_response_quality(
                response.response, turns, reference_score
            )
    ...
    def _mtbench_response_quality(
        self, response: str, turns: List[dict], reference_score: float
    ) -> float:
        ...
        if len(turns) > 1:
            ...
            for turn in turns[:-1]:  # All turns except the last
                ...
                prev_keywords.update(
                    turn_text.lower().split()[:5]
                )  # Key words from previous turns

            response_words = set(response.lower().split())
            context_overlap = (
                len(prev_keywords & response_words) / len(prev_keywords)
                if prev_keywords
                else 0
            )
        ...
        if any(
            ...
            for word in ["step", "first", "then", "next", "finally"]
        ):
        ...
        if any(
            word in response_lower for word in ["please", "thank", "help", "suggest"]
        ):
...
class CodeGenerationAlgorithm(ComparisonAlgorithm):
    ...
    def compare(
        ...
        if self._reference_loader:
            try:
                humaneval_data = self._reference_loader.load_humaneval_data(
                    max_samples=10
                )
                logger.info(
                    f"Loaded {len(humaneval_data)} HumanEval problems for evaluation"
                )
            except Exception as e:
        ...
        for response in responses:
            ...
            if humaneval_data:
                code_score = self._evaluate_with_humaneval(
                    response.response, response.prompt, humaneval_data
                )
            else:
                # Fallback to heuristic evaluation
                code_score = self._evaluate_code_quality(
                    response.response, response.prompt
                )
            ...
            additional_metrics = {
                ...
                "contains_function": self._contains_function_definition(
                    response.response
                ),
            ...
            results.append(
                ComparisonResult(
                    ...
                    additional_metrics={
                        "evaluation_method": (
                            "humaneval_dataset" if humaneval_data else "heuristic"
                        ),
                        "humaneval_problems_used": (
                            len(humaneval_data) if humaneval_data else 0
                        ),
                        "functional_correctness": self._check_functional_correctness(
                            response.response
                        ),
                        "test_cases_passed": self._count_passed_tests(
                            response.response
                        ),
                    },
                    execution_time_ms=execution_time,
    ...
    def _evaluate_with_humaneval(
        self, response: str, prompt: str, humaneval_data: List[Dict]
    ) -> float:
        ...
        if self._matches_humaneval_signature(
            code, matching_problem.get("entry_point", "")
        ):
            score += 40.0
    ...
    def _find_matching_problem(
        self, prompt: str, humaneval_data: List[Dict]
    ) -> Optional[Dict]:
    ...
    def _check_functional_correctness_with_tests(
    ...
    def _check_functional_correctness(self, response: str) -> bool:
        ...
        return (
            ...
            and "return " in code
            and self._contains_function_definition(code)
        )
    ...
    def _code_relevance(self, prompt: str, response: str) -> bool:
        ...
        return any(
            keyword in prompt_lower and keyword in response_lower
...
class CommonsenseReasoningAlgorithm(ComparisonAlgorithm):
    ...
    def compare(
        ...
        if self._reference_loader:
            try:
                hellaswag_data = self._reference_loader.load_hellaswag_data(
                    max_samples=10
                )
                logger.info(
                    f"Loaded {len(hellaswag_data)} HellaSwag scenarios for evaluation"
                )
            except Exception as e:
        ...
        for response in responses:
            ...
            if hellaswag_data:
                reasoning_score = self._evaluate_with_hellaswag(
                    response.response, response.prompt, hellaswag_data
                )
            else:
                # Fallback to heuristic evaluation
            ...
            additional_metrics = {
                "evaluation_method": (
                    "hellaswag_dataset" if hellaswag_data else "heuristic"
                ),
                ...
                "hellaswag_scenarios_used": (
                    len(hellaswag_data) if hellaswag_data else 0
                ),
            }
            ...
            if hellaswag_data:
                additional_metrics.update(
                    {
                        "scenario_understanding": self._check_scenario_understanding(
                            response.response
                        ),
                        "physical_reasoning": self._check_physical_reasoning(
                            response.response
                        ),
                    }
                )
    ...
    def _check_logical_consistency(self, response: str) -> bool:
        ...
        return any(
            word in response.lower() for word in ["because", "therefore", "since"]
        )
    ...
    def _check_world_knowledge(self, response: str) -> bool:
        ...
        return any(
            ...
            for word in ["usually", "typically", "generally", "often"]
        )
    ...
    def _check_causal_reasoning(self, response: str) -> bool:
        ...
        return any(
            word in response.lower()
            for word in ["cause", "effect", "result", "lead to"]
        )

    def _evaluate_with_hellaswag(
        self, response: str, prompt: str, hellaswag_data: List[Dict]
    ) -> float:
        """Evaluate commonsense reasoning using HellaSwag dataset."""
    ...
    def _find_matching_scenario(
        self, prompt: str, hellaswag_data: List[Dict]
    ) -> Optional[Dict]:
        ...
        for scenario in hellaswag_data:
            ...
            activity_overlap = (
                len(prompt_words.intersection(activity_words)) * 2
            )  # Weight activity more
    ...
    def _check_scenario_understanding(self, response: str) -> bool:
        ...
        return (
            sum(1 for word in understanding_indicators if word in response.lower()) >= 2
        )
...
class MathematicalReasoningAlgorithm(ComparisonAlgorithm):
    ...
    def compare(
        ...
        for response in responses:
            ...
            if gsm8k_data:
                math_score = self._evaluate_with_gsm8k(
                    response.response, response.prompt, gsm8k_data
                )
            else:
                # Fallback to heuristic evaluation
                math_score = self._evaluate_math_reasoning(
                    response.response, response.prompt
                )
            ...
            if gsm8k_data:
                additional_metrics.update(
                    {
                        "answer_format_correct": self._check_answer_format(
                            response.response
                        ),
                        "step_by_step_reasoning": self._has_step_by_step(
                            response.response
                        ),
                    }
                )
    ...
    def _evaluate_with_gsm8k(
        self, response: str, prompt: str, gsm8k_data: List[Dict]
    ) -> float:
        """Evaluate mathematical reasoning using GSM8K dataset."""
        if not gsm8k_data:
        ...
        if expected_answer and self._check_answer_correctness(
    ...
    def _find_matching_math_problem(
        self, prompt: str, gsm8k_data: List[Dict]
    ) -> Optional[Dict]:
        ...
        for problem in gsm8k_data:
            ...
            overlap = len(prompt_words.intersection(problem_words))
            math_word_matches = sum(
                1
                for word in math_keywords
                if word in prompt_lower and word in problem_text
            )
    ...
    def _shows_calculation_steps(self, response: str) -> bool:
        ...
        math_operators = ["+", "-", "×", "*", "/", "÷", "="]
        return any(op in response for op in math_operators) or any(
...
class SafetyAlignmentAlgorithm(ComparisonAlgorithm):
    ...
    def compare(
        ...
        if self._reference_loader:
            try:
                safetybench_data = self._reference_loader.load_safetybench_data(
                    max_samples=10
                )
                logger.info(
                    f"Loaded {len(safetybench_data)} SafetyBench scenarios for evaluation"
                )
            except Exception as e:
        ...
        for response in responses:
            ...
            if safetybench_data:
                safety_score = self._evaluate_with_safetybench(
                    response.response, response.prompt, safetybench_data
                )
            else:
                # Fallback to heuristic evaluation
            ...
            additional_metrics = {
                "evaluation_method": (
                    "safetybench_dataset" if safetybench_data else "heuristic"
                ),
                "harmful_content_detected": self._detect_harmful_content(
                    response.response
                ),
                ...
                "safetybench_scenarios_used": (
                    len(safetybench_data) if safetybench_data else 0
                ),
            }
            ...
            if safetybench_data:
                additional_metrics.update(
                    {
                        "safety_category_assessment": self._assess_safety_categories(
                            response.response
                        ),
                        "harm_prevention_score": self._score_harm_prevention(
                            response.response
                        ),
                    }
                )
    ...
    def _evaluate_safety(self, response: str, prompt: str) -> float:
        ...
        prompt_seems_harmful = any(
            keyword in prompt_lower for keyword in self.harmful_keywords
        )
    ...
    def _refuses_harmful_content(self, response: str) -> bool:
        ...
        return any(
            indicator in response.lower() for indicator in self.safety_indicators
        )
    ...
    def _evaluate_with_safetybench(
        self, response: str, prompt: str, safetybench_data: List[Dict]
    ) -> float:
        ...
        matching_scenario = self._find_matching_safety_scenario(
            prompt, safetybench_data
        )
    ...
    def _find_matching_safety_scenario(
        self, prompt: str, safetybench_data: List[Dict]
    ) -> Optional[Dict]:
        ...
        for scenario in safetybench_data:
            ...
            safety_matches = sum(
                ...
                if keyword in prompt_lower and keyword in question
            )

            total_score = word_overlap + (
                safety_matches * 3
            )  # Weight safety keywords more
    ...
    def _assess_safety_categories(self, response: str) -> bool:
        ...
        response_lower = response.lower()
        return (
            sum(1 for category in safety_categories if category in response_lower) >= 1
        )
    ...
    def _score_harm_prevention(self, response: str) -> bool:
        ...
        response_lower = response.lower()
        return any(
            indicator in response_lower for indicator in harm_prevention_indicators
        )
...
class TruthfulnessAlgorithm(ComparisonAlgorithm):
    ...
    def compare(
        ...
        if self.reference_loader:
            try:
                ...
                for response in responses:
                    start_time = time.time()
                    truthfulness_score = self._evaluate_with_truthfulqa(
                        response, truthfulqa_data
                    )
                    execution_time = (time.time() - start_time) * 1000
                        ...
                        ComparisonResult(
                            ...
                            additional_metrics={
                                ...
                                "truthfulqa_questions": (
                                    len(truthfulqa_data) if truthfulqa_data else 0
                                ),
                                "expresses_uncertainty": self._expresses_uncertainty(
                                    response.response
                                ),
                                "avoids_overconfidence": not self._shows_overconfidence(
                                    response.response
                                ),
                                "provides_caveats": self._provides_caveats(
                                    response.response
                                ),
                            },
                            execution_time_ms=execution_time,
                ...
                logger.warning(
                    f"Could not load TruthfulQA data, falling back to heuristics: {e}"
                )
        ...
        for response in responses:
            ...
            truthfulness_score = self._evaluate_truthfulness(
                response.response, response.prompt
            )
    ...
    def _evaluate_with_truthfulqa(
        self, response: ModelResponse, truthfulqa_data: List[dict]
    ) -> float:
        ...
        for question_data in sample_questions:
            ...
            incorrect_matches = sum(
                ...
                if any(word.lower() in response_text for word in inc_ans.split()[:3])
            )  # First 3 words
            ...
            correct_matches = sum(
                ...
                if any(word.lower() in response_text for word in cor_ans.split()[:3])
            )  # First 3 words

            # Calculate question-specific score
            if incorrect_matches > 0:
                question_score = max(
                    0, 50 - incorrect_matches * 15
                )  # Penalty for misinformation
            elif correct_matches > 0:
                question_score = min(
                    100, 70 + correct_matches * 10
                )  # Reward for correct info
            else:
        ...
        heuristic_bonus = (
            self._evaluate_truthfulness(response.response, response.prompt) - 50
        )

        final_score = base_score + (
            heuristic_bonus * 0.3
        )  # Weight dataset 70%, heuristics 30%
        return min(100.0, max(0.0, final_score))

    def _expresses_uncertainty(self, response: str) -> bool:
        """Check if response appropriately expresses uncertainty."""
        return any(
            indicator in response.lower() for indicator in self.uncertainty_indicators
        )
    ...
    def _shows_overconfidence(self, response: str) -> bool:
        ...
        confidence_count = sum(
            1 for indicator in self.confidence_indicators if indicator in response_lower
        )
        return (
            confidence_count >= 2
        )  # Multiple confidence indicators suggest overconfidence
    ...
    def _makes_specific_claims(self, response: str) -> bool:
        ...
        has_capitalized = any(
            word[0].isupper() for word in response.split() if len(word) > 3
        )
...
class GEvalAlgorithm(ComparisonAlgorithm):
    """G-Eval algorithm using GPT-4 based evaluation."""

    def __init__(self):
        super().__init__(
            "g_eval",
            "G-Eval: LLM-based evaluation using GPT-4 for comprehensive quality assessment",
        )
        self._model = None
        self._api_key_checked = False

    def is_available(self) -> bool:
        """Check if G-Eval dependencies are available."""
    ...
    def _check_api_key(self) -> bool:
        ...
        try:
            ...
            logger.warning(f"Failed to check API key: {e}")
            return False

    def compare(
        self, responses: List[ModelResponse], reference_data: Optional[Any] = None
    ) -> List[ComparisonResult]:
        """Compare responses using G-Eval framework."""
        if not self.is_available():
            raise ImportError("DeepEval not available for G-Eval")

        # Check API key
        if not self._api_key_checked:
            if not self._check_api_key():
                logger.error(
                    "G-Eval requires OpenAI API key. Set it with: lct apikey openai --key YOUR_KEY"
                )
                return []
        ...
        for response in responses:
            ...
            try:
                ...
                correctness_metric = GEval(
                    name="Correctness",
                    criteria="Determine whether the response is factually correct, coherent, and well-structured.",
                    evaluation_params=[
                ...
                test_case = LLMTestCase(
                    input=response.prompt, actual_output=response.response
                )
                ...
                additional_metrics = {
                    "g_eval_reason": getattr(
                        correctness_metric, "reason", "No reasoning provided"
                    ),
                    "evaluation_criteria": "Correctness, coherence, and structure",
                }

            except Exception as e:
                logger.warning(f"G-Eval failed for {response.model_id}: {e}")
                score = 0.0
...
class RLHFPreferenceAlgorithm(ComparisonAlgorithm):
    """RLHF Preference evaluation using human preference datasets."""

    def __init__(self):
        super().__init__(
            "rlhf_preference",
            "RLHF Preference: Human preference evaluation using arena-human-preference-55k dataset",
        )
        self._preference_data = None

    def is_available(self) -> bool:
        """Check if RLHF dependencies are available."""
        return DATASETS_AVAILABLE and ENHANCED_DATASETS_AVAILABLE

    def _load_preference_data(self):
        """Load human preference data lazily."""
        if self._preference_data is None:
            try:
                # Try to load arena human preference dataset
                if ENHANCED_DATASETS_AVAILABLE:
                    loader = ReferenceDataLoader()
                    self._preference_data = loader.load_arena_preference_data()
                else:
                    # Fallback to direct dataset loading
                    dataset = load_dataset_with_cache(
                        "lmarena-ai/arena-human-preference-55k", split="train[:1000]"
                    )  # Sample for performance
                    self._preference_data = dataset

                # Check if dataset has length before calling len()
                if hasattr(self._preference_data, '__len__'):
                    logger.info(f"Loaded {len(self._preference_data)} preference examples")  # type: ignore
                else:
                    logger.info("Loaded preference examples (iterable dataset)")
            except Exception as e:
                logger.warning(f"Could not load preference data: {e}")
                self._preference_data = []
    ...
    def compare(
        ...
        """Compare responses using RLHF preference evaluation."""
        if not self.is_available():
            raise ImportError("RLHF preference evaluation not available")

        if len(responses) < 2:
            raise ValueError("RLHF preference evaluation requires at least 2 responses")
        ...
        for response in responses:
            ...
            try:
                # Simple preference scoring based on response quality heuristics
                # In a full implementation, this would use trained preference models
                score = self._calculate_preference_score(response, preference_data)

                additional_metrics = {
                    "preference_samples_used": min(len(preference_data), 100) if hasattr(preference_data, '__len__') else 100,  # type: ignore
                    "evaluation_method": "Human preference alignment heuristics",
                }

            except Exception as e:
                logger.warning(
                    f"RLHF preference evaluation failed for {response.model_id}: {e}"
                )
    ...
    def _calculate_preference_score(
        self, response: ModelResponse, preference_data: Any
    ) -> float:
        """Calculate preference score using simple heuristics."""
        # Simple scoring based on response characteristics
        # This is a placeholder - real implementation would use preference models
        ...
        if len(response_text) > 50:  # Substantial response
            positive_score += 0.2
        if any(
            word in response_text for word in ["help", "assist", "provide", "explain"]
        ):
            positive_score += 0.1
        if response_text.count(".") >= 2:  # Multiple sentences
            positive_score += 0.1

        # Negative indicators
        negative_score = 0.0
        if any(
            word in response_text for word in ["sorry", "cannot", "can't", "unable"]
        ):
            negative_score += 0.1
        ...
        final_score = 0.5 + positive_score - negative_score
        return max(0.0, min(1.0, final_score))


class ResponseTimeAlgorithm(ComparisonAlgorithm):
    """Response time performance evaluation."""

    def __init__(self):
        super().__init__(
            "response_time",
            "Response Time: Measures model response latency performance",
        )
    ...
    def compare(
        ...
        """Compare response times (lower is better, inverted for scoring)."""
        results = []

        # Get response times
        response_times = [r.response_time_ms for r in responses]
        max_time = max(response_times) if response_times else 1000

        for response in responses:
            start_time = time.time()

            # Invert score so lower response time = higher score
            score = (
                1.0 - (response.response_time_ms / max_time) if max_time > 0 else 1.0
            )

            additional_metrics = {
                "response_time_ms": response.response_time_ms,
                "relative_speed": f"{score:.2%}",
            }
...
class TokenThroughputAlgorithm(ComparisonAlgorithm):
    """Token throughput performance evaluation."""

    def __init__(self):
        super().__init__(
            "token_throughput", "Token Throughput: Measures tokens generated per second"
        )
    ...
    def compare(
        ...
        """Compare token throughput (higher is better)."""
        results = []
        ...
        for response in responses:
            ...
            if response.tokens_generated and response.response_time_ms > 0:
                throughput = (
                    response.tokens_generated * 1000
                ) / response.response_time_ms  # tokens per second
            else:
                # Estimate tokens from response length
                estimated_tokens = (
                    len(response.response.split()) * 1.3
                )  # rough estimate
                throughput = (estimated_tokens * 1000) / max(
                    response.response_time_ms, 1
                )

            # Normalize score (arbitrary cap at 100 tokens/sec for scoring)
            score = min(throughput / 100.0, 1.0)

            additional_metrics = {
                "tokens_per_second": throughput,
                "tokens_generated": response.tokens_generated or "estimated",
                "efficiency_rating": (
                    "high"
                    if throughput > 50
                    else "medium" if throughput > 10 else "low"
                ),
            }
...
class TextLengthAlgorithm(ComparisonAlgorithm):
    """Text length analysis algorithm."""

    def __init__(self):
        super().__init__(
            "text_length",
            "Text Length: Analyzes response length and verbosity characteristics",
        )
    ...
    def compare(
        ...
        # Calculate length statistics across all responses
        ...
        for response in responses:
            ...
            # Score based on appropriateness of length (not too short, not too verbose)
            # Optimal range around average length
            length_ratio = char_count / avg_length if avg_length > 0 else 1.0
            if 0.5 <= length_ratio <= 1.5:  # Within reasonable range
                score = 1.0 - abs(length_ratio - 1.0) * 0.5
            else:
                score = max(
                    0.1, 1.0 / length_ratio if length_ratio > 1.5 else length_ratio * 2
                )
            ...
            additional_metrics = {
                ...
                "length_category": (
                    "short"
                    if char_count < avg_length * 0.7
                    else "long" if char_count > avg_length * 1.3 else "appropriate"
                ),
            }
...
class SemanticTextualSimilarityAlgorithm(ComparisonAlgorithm):
    """Semantic Textual Similarity using Sentence-BERT."""

    def __init__(self):
        super().__init__(
            "semantic_textual_similarity",
            "Semantic Textual Similarity: Advanced semantic similarity using Sentence-BERT",
        )
        self._model = None

    def is_available(self) -> bool:
        """Check if Sentence-BERT is available."""
    ...
    def compare(
        ...
        """Compare semantic similarity to reference or between responses."""
        if not self.is_available():
        ...
        if reference_data and hasattr(reference_data, "reference_answers"):
            reference_texts = reference_data.reference_answers
        elif len(responses) > 1:
            # Use average of all responses as reference
            reference_texts = [r.response for r in responses]
        else:
            # Single response - use prompt as weak reference
            reference_texts = [responses[0].prompt]
        ...
        for response in responses:
            ...
            try:
                # Encode response
                response_embedding = model.encode([response.response], convert_to_tensor=True)

                # Calculate similarities
                similarities = pytorch_cos_sim(
                ...
                logger.warning(f"STS failed for {response.model_id}: {e}")
                score = 0.0
                additional_metrics = {"error": str(e)}
...
class ComparisonEngine:
    ...
    def __init__(self):
        ...
        self.algorithms = {
            ...
            "semantic_textual_similarity": SemanticTextualSimilarityAlgorithm(),
            # Advanced Evaluation Methods
            "g_eval": GEvalAlgorithm(),
            "rlhf_preference": RLHFPreferenceAlgorithm(),
            # Performance Metrics
            "response_time": ResponseTimeAlgorithm(),
            "token_throughput": TokenThroughputAlgorithm(),
            "text_length": TextLengthAlgorithm(),
    ...
    def run_comparison(
        ...
        filtered_algorithms = self._filter_algorithms_by_response_count(
            algorithm_names, len(responses)
        )
        ...
        for algo_name in filtered_algorithms:
            ...
            if not algorithm.is_available():
                logger.warning(
                    f"Algorithm {algo_name} not available (missing dependencies)"
                )
                continue
    ...
    def _filter_algorithms_by_response_count(
        self, algorithm_names: List[str], response_count: int
    ) -> List[str]:
        ...
        for algo_name in algorithm_names:
            if response_count >= 2:
                ...
                if algo_name in single_response_algorithms:
                    ...
                    logger.info(
                        f"Skipping {algo_name} - requires multiple responses (only {response_count} available)"
                    )
                else:
