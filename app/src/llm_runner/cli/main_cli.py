...
from ..discovery.hf_model_discovery import HuggingFaceModelDiscovery, ModelSearchCriteria
...
@click.option("--sort-by", default="downloads", type=click.Choice(['downloads', 'created_at', 'modified_at', 'size', 'relevance']),
...
@click.option("--popular", is_flag=True, help="Show only popular models (>1000 downloads)")
@click.option("--recent", is_flag=True, help="Show only recently updated models (last 30 days)")
@click.option("--interactive", "-i", is_flag=True, help="Launch interactive selection after search")
def search(task, language, author, min_downloads, max_downloads, min_size, max_size, tags, license, query, fuzzy, sort_by, limit, save, compatible_only, lightweight, popular, recent, interactive):
    ...
    try:
        ...
        if recent:
            ...
            console.print("[dim]Recent filter applied (implementation may vary by API)[/dim]")
        ...
        if len(models) < original_count:
            console.print(f"[dim]Filtered from {original_count} to {len(models)} models[/dim]")
...
def _interactive_model_selection(models):
    ...
    try:
        ...
        console.print("[dim]Commands: 'a' = select all, 'c' = clear selection, 'q' = quit, number = toggle selection[/dim]\n")
        ...
        while True:
            ...
            for i, model in enumerate(models):
                ...
                console.print(f"{status} [{i+1:2d}] {model.id} ({size_str}, {downloads_str} downloads)")
            
            console.print(f"\n[green]Selected: {len(selected_models)} models[/green]")
            choice = input("\nEnter choice (1-{}, 'a', 'c', 'q', or Enter to finish): ".format(len(models))).strip().lower()
...
def select(from_search, task, max_size, limit):
    ...
    try:
        ...
        if not models:
            console.print("[yellow]No models found. Try adjusting criteria or running a search first.[/yellow]")
            return
...
def info(model_id, save):
    ...
    try:
        ...
        console.print(f"[bold]Size:[/bold] {model.size_gb:.1f} GB" if model.size_gb else "Size: Unknown")
        console.print(f"[bold]Compatible:[/bold] {'✓ Yes' if model.compatible else '✗ No'}")
        
        if not model.compatible and model.compatibility_notes:
            console.print(f"[yellow]Compatibility Notes:[/yellow] {model.compatibility_notes}")
...
@click.option("--algorithms", default="response_time,text_length", 
              help="Comma-separated algorithm names")
@click.option("--interactive", "-i", is_flag=True, help="Interactive algorithm selection")
@click.option("--repetitions", default=3, type=int, help="Number of repetitions per test")
@click.option("--output-dir", help="Output directory for results")
@click.option("--max-memory", default=8.0, type=float, help="Maximum memory for models (GB)")
...
@click.option("--energy-profiler", default="none", 
              type=click.Choice(['none', 'powerjoular', 'energibridge', 'rapl', 'codecarbon']),
              help="Energy profiler to use")
@click.option("--energy-profiling", "-e", is_flag=True, help="Enable energy profiling with PowerJoular")
def init(name, models, models_file, prompts, prompts_file, algorithms, interactive, repetitions, output_dir, max_memory, max_length, temperature, energy_profiler, energy_profiling):
    ...
    if interactive:
        ...
        algorithm_categories = {
            "1": ("Performance Metrics", ["response_time", "token_throughput", "text_length"]),
            "2": ("N-gram Based (need references)", ["bleu", "rouge"]), 
            "3": ("Semantic Similarity (need references)", ["bert_score", "semantic_similarity", "semantic_textual_similarity"]),
            "4": ("Human-Aligned Evaluation", ["pairwise_comparison", "llm_as_judge"]),
            "5": ("Advanced Frameworks", ["g_eval", "rlhf_preference"]),
            "6": ("Task-Specific", ["code_generation", "commonsense_reasoning", "mathematical_reasoning", "safety_alignment", "truthfulness"])
        }
        
        selected_algorithms = []
        
        for cat_num, (cat_name, cat_algorithms) in algorithm_categories.items():
            available_in_category = [alg for alg in cat_algorithms if alg in available_algorithms]
            if available_in_category:
                console.print(f"[yellow]{cat_num}. {cat_name}[/yellow]")
                for i, alg in enumerate(available_in_category, 1):
                    console.print(f"   {i}. {alg}")
                
                selection = click.prompt(f"Select algorithms from {cat_name} (comma-separated numbers, 'all', or 'skip')", default='skip')
                ...
                if selection.lower() == 'all':
                    ...
                    try:
                        ...
                        selected_algorithms.extend([available_in_category[i] for i in indices if 0 <= i < len(available_in_category)])
    ...
    if energy_profiling and energy_profiler == "none":
        energy_profiler = "powerjoular"  # Default to PowerJoular if -e flag is used
    ...
    if invalid_algorithms:
        console.print(f"[yellow]Warning: Invalid algorithms (will be skipped): {', '.join(invalid_algorithms)}[/yellow]")
        algorithm_list = [alg for alg in algorithm_list if alg in available_algorithms]

    console.print(f"[green]Available algorithms: {', '.join(available_algorithms.keys())}[/green]")
    ...
    if energy_profiler != "none":
        console.print(f"[green]Energy profiling enabled with: {energy_profiler}[/green]")
...
'''
        ...
        config_import = "from llm_runner.configs.llm_comparison_config import LLMComparisonConfig"
...
'''
    ...
    console.print(f"[bold]Total runs:[/bold] {len(model_ids) * len(prompt_list) * repetitions}")
    
    console.print(f"\n[bold blue]To run the experiment:[/bold blue]")
    if energy_profiler == "powerjoular":
        console.print(f"[cyan]# PowerJoular requires sudo access[/cyan]")
        relative_path = Path(config_path).parent.relative_to(Path.cwd())
        console.print(f"[cyan]cd {relative_path}[/cyan]")
        console.print(f"[cyan]sudo -E env PATH=$PATH PYTHONPATH=../../../app/src python3 -m experiment_runner RunnerConfig.py[/cyan]")
...
def run(config_file, dry_run):
    ...
    try:
        ...
        result = subprocess.run(cmd, capture_output=False, text=True, cwd=str(app_src_path), env=env)
        
        if result.returncode == 0:
            console.print("[green]✓ Experiment completed successfully![/green]")
        else:
            console.print(f"[red]✗ Experiment failed with return code {result.returncode}[/red]")
...
def setup():
    ...
    if not setup_script.exists():
        ...
        console.print("The setup script should be available. Please check the repository.")
        return
    ...
    console.print("• PowerJoular (from source)")
    console.print("• RAPL access configuration") 
    console.print("• Python dependencies")
    console.print("• Build tools (GNAT, gprbuild)")
    ...
    try:
        ...
        console.print("1. Logout/login to activate group permissions")
        console.print("2. Run verification: [cyan]lct verify[/cyan]")
        console.print("3. Test energy profiling: [cyan]lct init --energy-profiler powerjoular[/cyan]")
...
def verify():
    ...
    if not verify_script.exists():
        ...
        console.print("Run '[cyan]lct setup[/cyan]' first to create verification script")
        return
    ...
    try:
        ...
        result = subprocess.run(['python3', str(verify_script)], 
                              cwd=verify_script.parent)
        
        if result.returncode == 0:
            console.print("\n[green]✅ All energy profiling tools verified![/green]")
            console.print("\n[bold]Ready for energy profiling experiments![/bold]")
            console.print("Create an experiment: [cyan]lct init --name test --energy-profiler powerjoular[/cyan]")
        else:
            console.print(f"\n[yellow]⚠️  Some tools need attention (exit code: {result.returncode})[/yellow]")
            console.print("Check the output above and run '[cyan]lct setup[/cyan]' if needed")
...
def status():
    ...
    if experiment_runner_path.exists():
        ...
        console.print("   LCT should be run from within the correct directory structure")
    ...
    try:
        result = subprocess.run(["which", "powerjoular"], check=True, capture_output=True)
        console.print("✅ PowerJoular: [green]Available[/green]")
        console.print(f"   Path: {result.stdout.decode().strip()}")
    except (subprocess.CalledProcessError, FileNotFoundError):
        console.print("❌ PowerJoular: [red]Not installed[/red]")
        console.print("   Install: [cyan]lct setup[/cyan]")
...
def dependencies():
    ...
    if deps_to_install:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                console.print("Run [cyan]lct algorithms[/cyan] to see available algorithms")
            else:
    ...
    console.print("For complete energy profiling setup including PowerJoular:")
    console.print("• Full setup: [cyan]lct setup[/cyan]")
...
def algorithms():
    ...
    for category, algorithms_dict in categories.items():
        ...
        table.add_column("Reference Needed", style="blue", width=15)
        ...
        for name, description in algorithms_dict.items():
            ...
            requires_ref = "Yes" if "requires reference" in description.lower() else "No"
            ref_style = "yellow" if requires_ref == "Yes" else "green"
            
            table.add_row(
                name, 
                description.replace(" (requires reference)", ""),
                f"[{status_style}]{status}[/{status_style}]",
                f"[{ref_style}]{requires_ref}[/{ref_style}]"
            )
    ...
    if available_count < total_algorithms:
        ...
        console.print("pip install sacrebleu rouge-score bert-score sentence-transformers evaluate datasets numpy")
    ...
    console.print("\n# Task-specific benchmarks")
    console.print("lct init --algorithms \"code_generation,mathematical_reasoning,safety_alignment\"")
    console.print("\n# Comprehensive evaluation")
    console.print("lct init --algorithms \"response_time,bleu,rouge,g_eval,safety_alignment,truthfulness\"")
...
@auth.command()
@click.option('--token', prompt='HuggingFace Token', hide_input=True, help='Your HuggingFace API token')
...
def setup(token, permanent):
    ...
    if not token.startswith('hf_'):
        console.print("[red]❌ Invalid token format. HuggingFace tokens should start with 'hf_'[/red]")
    ...
    try:
        ...
        console.print(f"[green]✅ Token valid! Authenticated as: {user_info['name']}[/green]")
    except Exception as e:
    ...
    if permanent:
        try:
            ...
            lines = [line for line in lines if not line.strip().startswith('export HF_TOKEN=')]
            ...
            console.print("[yellow]💡 Restart your terminal or run 'source ~/.bashrc' to load in new sessions[/yellow]")
...
def status():
    ...
    if hf_token:
        console.print(f"[green]✅ HF_TOKEN found: {hf_token[:8]}...{hf_token[-4:]}[/green]")
        token_to_test = hf_token
    elif hf_hub_token:
        console.print(f"[green]✅ HUGGINGFACE_HUB_TOKEN found: {hf_hub_token[:8]}...{hf_hub_token[-4:]}[/green]")
    ...
    try:
        ...
        console.print("1. Check token validity at: https://huggingface.co/settings/tokens")
        console.print("2. Run: lct auth setup")
...
def remove():
    ...
    if 'HUGGINGFACE_HUB_TOKEN' in os.environ:
        ...
        console.print("[green]✅ Removed HUGGINGFACE_HUB_TOKEN from current session[/green]")
    ...
    try:
        ...
        if bashrc_path.exists():
            ...
            lines = [line for line in lines if not (
                line.strip().startswith('export HF_TOKEN=') or 
                line.strip().startswith('export HUGGINGFACE_HUB_TOKEN=')
            )]
...
@cli.command()
@click.option("--quick", is_flag=True, help="Run quick diagnostic (skip model loading test)")
@click.option("--model", default="distilgpt2", help="Test model for loading capability (default: distilgpt2)")
...
def diagnostic(quick, model, verbose):
    ...
    try:
        ...
        if memory_gb >= 8:
            console.print(f"[green]✅ RAM:[/green] {memory_gb:.1f} GB total, {memory_available_gb:.1f} GB available ({100-memory_percent:.1f}% free)")
        elif memory_gb >= 4:
            console.print(f"[yellow]⚠️  RAM:[/yellow] {memory_gb:.1f} GB total, {memory_available_gb:.1f} GB available (may limit model size)")
        else:
            console.print(f"[red]❌ RAM:[/red] {memory_gb:.1f} GB total - insufficient for most LLM experiments")
            
        results['memory'] = {
            'status': 'ok' if memory_gb >= 8 else 'warning' if memory_gb >= 4 else 'error',
            'total_gb': memory_gb,
        ...
        if disk_free_gb >= 10:
            ...
            console.print(f"[yellow]⚠️  Disk Space:[/yellow] {disk_free_gb:.1f} GB free (may limit model downloads)")
        else:
            console.print(f"[red]❌ Disk Space:[/red] {disk_free_gb:.1f} GB free - insufficient for model downloads")
            
        results['disk'] = {
            'status': 'ok' if disk_free_gb >= 10 else 'warning' if disk_free_gb >= 5 else 'error',
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            console.print(f"[green]✅ NVIDIA GPU:[/green] {gpu_name} ({gpu_memory:.1f} GB VRAM)")
            results['gpu'] = {'status': 'ok', 'type': 'cuda', 'name': gpu_name, 'memory_gb': gpu_memory}
            gpu_found = True
        else:
        ...
        if platform.system() == "Linux":
            ...
            if 'Intel' in result.stdout and ('VGA' in result.stdout or 'Display' in result.stdout):
                ...
                if intel_gpus and verbose:
                    console.print(f"[blue]ℹ️  Intel GPU detected: {intel_gpus[0].split(':')[-1].strip()}[/blue]")
    except Exception:
        pass
    
    if not gpu_found:
        console.print("[yellow]⚠️  No dedicated GPU detected - using CPU only (slower performance)[/yellow]")
    ...
    if hf_token:
        masked_token = hf_token[:8] + "..." + hf_token[-4:] if len(hf_token) > 12 else hf_token[:4] + "..."
        ...
        try:
            ...
            console.print(f"[green]✅ Token valid for user:[/green] {user_info.get('name', 'Unknown')}")
            results['auth'] = {'status': 'ok', 'user': user_info.get('name', 'Unknown')}
        except ImportError:
            console.print("[yellow]⚠️  huggingface_hub not available for token validation[/yellow]")
    ...
    if not quick and 'transformers' not in missing_packages:
        ...
        try:
            ...
            console.print(f" [green]✅ Model loading successful[/green] ({load_time:.1f}s)")
            results['model_test'] = {'status': 'ok', 'load_time': load_time}
                
        except ImportError as e:
            console.print(f"[red]❌ Missing dependencies for model test:[/red] {e}")
            results['model_test'] = {'status': 'error', 'message': f'Missing dependencies: {e}'}
        except Exception as e:
