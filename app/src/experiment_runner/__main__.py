...
def load_and_get_config_file_as_module(args: List[str]):
    ...
    if spec is None:
        raise ImportError(f"Could not load module spec from {args[1]}")
        ...
        if isinstance(node, (ast.AsyncFunctionDef, ast.FunctionDef, ast.ClassDef, ast.Module)) and ast.get_docstring(node) is not None:
            if node.body and isinstance(node.body[0], ast.Expr):
                ...
                if hasattr(expr_node, 'value'):
                    ...
                    elif hasattr(docstring_node, 's') and hasattr(docstring_node, '__dict__'):  # Legacy ast.Str support
                        try:
                            docstring_node.s = ''  # type: ignore
