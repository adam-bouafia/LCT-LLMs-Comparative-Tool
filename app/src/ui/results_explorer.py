...
class ResultsExplorer:
    ...
    def scan_experiments(self) -> List[Dict]:
        ...
        return sorted(
            experiments, key=lambda x: x.get("last_modified", ""), reverse=True
        )
    ...
    def view_experiment_details(self, exp_info: Dict):
        ...
        if csv_file.exists():
            ...
            if Confirm.ask(
                "\n[bold cyan]Would you like to view the full CSV content?[/bold cyan]",
                default=False,
            ):
    ...
    def show_csv_preview(self, csv_file: Path):
        ...
        try:
            ...
            # Show summary stats
            completed_runs = sum(
                1 for row in rows if row.get("__done", "").upper() == "DONE"
            )
    ...
    def view_csv_content(self, csv_file: Path):
        ...
        try:
            ...
            while True:
                self.console.clear()
                self.console.print(
                    f"[bold blue]📋 CSV Content: {csv_file.name}[/bold blue]"
                )
    ...
    def show_full_csv_table(self, rows: List[Dict], columns: List[str]):
        ...
        if len(rows) > 50:
            self.console.print(
                f"[dim]Note: Showing {len(display_rows)} of {len(rows)} rows[/dim]"
            )
    ...
    def show_selected_columns(self, rows: List[Dict], columns: List[str]):
        ...
        selection = Prompt.ask(
            "\nEnter column numbers (e.g., 1,3,5) or 'all'", default="all"
        )
        ...
        if selection.lower() == "all":
            ...
            try:
                ...
                selected_columns = [
                    columns[i] for i in indices if 0 <= i < len(columns)
                ]
            except:
    ...
    def filter_by_status(self, rows: List[Dict], columns: List[str]):
        ...
        status_columns = [
            col for col in columns if "done" in col.lower() or "status" in col.lower()
        ]
        ...
        selection = Prompt.ask(
            "Select filter value",
            choices=[str(i) for i in range(1, len(unique_values) + 1)],
        )

        try:
            filter_value = unique_values[int(selection) - 1]
            filtered_rows = [
                row for row in rows if str(row.get(status_col, "")) == filter_value
            ]
            ...
            if filtered_rows:
                ...
                if len(filtered_rows) > 20:
                    self.console.print(
                        f"[dim]Showing first 20 of {len(filtered_rows)} filtered rows[/dim]"
                    )
    ...
    def show_csv_statistics(self, rows: List[Dict], columns: List[str]):
        ...
        if status_cols and rows:
            ...
            completed = sum(
                1 for row in rows if str(row.get(status_col, "")).upper() == "DONE"
            )
            stats_table.add_row(
                "Completed Runs", f"{completed} ({completed/len(rows)*100:.1f}%)"
            )
        ...
        if numeric_cols:
            ...
            for col in numeric_cols[:5]:  # Show first 5 numeric columns
                ...
                if values:
                    self.console.print(
                        f"  {col}: avg={sum(values)/len(values):.2f}, min={min(values):.2f}, max={max(values):.2f}"
                    )
    ...
    def search_csv_data(self, rows: List[Dict], columns: List[str]):
        ...
        if matching_rows:
            ...
            if len(matching_rows) > 15:
                self.console.print(
                    f"[dim]Showing first 15 of {len(matching_rows)} matching rows[/dim]"
                )

        Prompt.ask("\nPress Enter to continue")

    def export_filtered_csv(
        self, original_file: Path, rows: List[Dict], columns: List[str]
    ):
        ...
        if choice == "2":
            ...
            if status_cols:
                ...
                export_rows = [
                    row
                    for row in rows
                    if str(row.get(status_col, "")).upper() == "DONE"
                ]
                suffix = "_completed"
        ...
        try:
            ...
            self.console.print(
                f"[green]✅ Exported {len(export_rows)} rows to {export_file}[/green]"
            )
    ...
    def delete_all_experiments(self, experiments: List[Dict]):
        ...
        if experiments_with_data > 0:
            if not Confirm.ask(
                f"[red]{experiments_with_data} experiments contain results data. Really delete?[/red]"
            ):
                self.console.print("[yellow]Deletion cancelled.[/yellow]")
                return
        ...
        if Confirm.ask("Create backup archive before deleting all experiments?"):
            try:
                ...
                backup_created = True
                self.console.print(
                    f"[green]✅ Backup created: {archive_name}.zip[/green]"
                )
        ...
        if deleted_count == len(experiments):
            self.console.print(
                f"[green]✅ Successfully deleted all {deleted_count} experiments![/green]"
            )
        else:
            self.console.print(
                f"[yellow]⚠️  Deleted {deleted_count} of {len(experiments)} experiments[/yellow]"
            )
    ...
    def export_all_experiments(self, experiments: List[Dict]):
        ...
        self.console.print(
            f"Found {len(exportable_experiments)} experiments with exportable data"
        )

        if not exportable_experiments:
            self.console.print(
                "[yellow]No experiments contain exportable data (CSV/JSON files).[/yellow]"
            )
            return
        ...
        for exp in exportable_experiments:
            ...
            export_table.add_row(
                exp["name"], ", ".join(files), str(exp["runs"]), exp["size"]
            )
        ...
        choice = Prompt.ask(
            "Choose export option", choices=["0", "1", "2", "3", "4", "5"]
        )
        ...
        for exp in experiments:
            ...
            if exp["csv_exists"]:
                try:
                    ...
                    summary["aggregate_stats"]["total_completed_runs"] += exp_summary[
                        "csv_analysis"
                    ]["completed_runs"]
    ...
    def delete_experiment(self, exp_info: Dict):
        ...
        if exp_info["runs"] > 10:
            if not Confirm.ask(
                f"[red]This experiment has {exp_info['runs']} runs. Really delete?[/red]"
            ):
                self.console.print("[yellow]Deletion cancelled.[/yellow]")
                return
        ...
        try:
            ...
            if Confirm.ask("Create a backup before deleting?"):
                backup_name = (
                    f"{exp_name}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
                )
            ...
            shutil.rmtree(exp_path)
            self.console.print(
                f"[green]✅ Successfully deleted experiment '{exp_name}'[/green]"
            )
    ...
    def export_results(self, exp_info: Dict):
        ...
        if not has_csv:
            self.console.print(
                "[red]No results to export (no run_table.csv found)[/red]"
            )
            return
        ...
        try:
            if choice == "1":
                ...
                self.console.print(
                    "[yellow]Excel export requires pandas and openpyxl. Install with: pip install pandas openpyxl[/yellow]"
                )
    ...
    def run(self):
        ...
        while True:
            ...
            if not experiments:
                self.console.print(
                    "[yellow]No experiments found in the experiments directory.[/yellow]"
                )
                self.console.print(
                    "Make sure you're running this from the llm-experiment-runner directory."
                )
                break
            ...
            if choice == "q":
                ...
                try:
                    ...
                    if 0 <= exp_id < len(experiments):
                        ...
                        if csv_file.exists():
                            ...
                            self.console.print(
                                f"[red]No CSV file found for experiment '{exp_info['name']}'[/red]"
                            )
                    ...
                    self.console.print(
                        "[red]Invalid format. Use 'c1', 'c2', etc.[/red]"
                    )
                ...
                try:
                    ...
                    self.console.print(
                        "[red]Invalid format. Use 'd1', 'd2', etc.[/red]"
                    )
                ...
                try:
                    ...
                    self.console.print(
                        "[red]Invalid format. Use 'e1', 'e2', etc.[/red]"
                    )
            else:
                try:
                    exp_id = int(choice) - 1
                    ...
                    self.console.print(
                        "[red]Please enter a valid number or command[/red]"
                    )
